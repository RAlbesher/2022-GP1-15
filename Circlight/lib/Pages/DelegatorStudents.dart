...
class _DelegatorStudent extends State<DelegatorStudent>
  ...
  //var DelegID = FirebaseAuth.instance.currentUser!.uid.toString();
  var DelegID = "F18QfhnW1ROaOW3ojQ2iNBSrmsE2";
...
/*
  ...
  RequestCount() {
    ReqDel.get().then((snapshot) => snapshot.docs.forEach((document) {
          ...
          } else if (StudentRequests.isEmpty == true) {
            ShowRequest = false;
            setState(() {
              NoRequest = false;
            });
  ...
  RequestCount2() {
    ReqDel.get().then((snapshot) => snapshot.docs.forEach((document) {
          ...
          } else if (ParentRequests.isEmpty == true) {
            ShowRequest = false;
            setState(() {
              NoRequest2 = false;
            });
  ...
  Future GetStudentName(snap) async {
    ...
    for (var j = 0; j < StudentID.length; j++) {
      ...
      await ColStudent.doc(StudentID[j]).get().then((value) {
        ...
        StudentName.add(
            "ID" + " " + value["NationalID"] + " " + "الطالب " + value["Name"]);
        // print("Starteddd");
